...
export const getListOfUsers = () => async (dispatch, getState) => {
  try {
    dispatch({ type: GET_LIST_OF_USERS_REQUEST });
    ...
    const { data } = await axios.get(`/api/users/unverified`, config);

    dispatch({ type: GET_LIST_OF_USERS_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: GET_LIST_OF_USERS_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
...
export const verifyUser = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(`/api/users/verify/${id}`, {}, config);
