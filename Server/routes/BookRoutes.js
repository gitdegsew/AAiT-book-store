...
productRoute.delete(
  ...
  asyncHandler(async (req, res) => {
    const product = await Product.findById(req.params.id);
    if (product) {
      const review = await product.reviews.id(req.params.revid);
      if (review) {
...
productRoute.post(
  ...
  admin,
  asyncHandler(async (req, res) => {
    const { title, author, description, bookcode, image, countInStock } =
      req.body;
...
productRoute.put(
  ...
  admin,
  asyncHandler(async (req, res) => {
    const { title, author, description, image, countInStock, bookcode } =
      req.body;
    const product = await Product.findById(req.params.id);
    if (product) {
